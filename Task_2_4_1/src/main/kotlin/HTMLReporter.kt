...
fun generateHtmlReport(students: List<Student>, tasks: List<Task>, fileName: String) {
                    ...
                    for (student in groupStudents) {
                        ...
                        for ((index, res) in results.withIndex()) {
                            tr {
                                ...
                                td(classes = if (res.build) "ok" else "fail") {
                                    +if (res.build) "✔" else "✘"
                                }
                                td(classes = if (res.checkstyle) "ok" else "fail") {
                                    +if (res.checkstyle) "✔" else "✘"
                                }
                                td(classes = if (res.test) "ok" else "fail") {
                                    +if (res.test) "✔" else "✘"
                                }
                                if (index == 0) {
                                    td { +student.getMark().toString() }
                                } else {
                                    td {  }
                                }
                            }
